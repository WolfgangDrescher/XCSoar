...
ButtonLook::Initialise(const Font &_font, bool dark_mode)
{
  ...
  if (dark_mode) {
    ...
    /* a smaller step from the face than the light mode ring takes: a
       button on the map has the terrain behind it, and a mid gray
       outline lands between the dark face and a bright slope instead
       of edging it */
    ...
    /* on a dithered display the focused button carries the pattern of
       a light gray, the only value between the two levels such a
       display has.  A gray scale display has the shades, but not the
       hue that lets the primary fill carry white text: the gray of
       the same lightness reads as a hole in the page, so the face
       becomes the first card below the white one and keeps the black
       caption */
