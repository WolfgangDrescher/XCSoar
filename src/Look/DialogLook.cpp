...
DialogLook::Initialise(bool _dark_mode)
{
     ...
     list.focused in dark is darker (XCSOAR_DARK) */
  {
    if (dark_mode && IsDithered()) {
      ...
      /* without a hue the states have to line up on the lightness
         scale, one step further from the white face of an enabled
         button with every one: the selected card sits on the level
         that outlines that white face.  A dithered display has no
         such scale and keeps the two levels below */
