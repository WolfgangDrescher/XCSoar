...
DrawBoxShadow([[maybe_unused]] const PixelRect &rc) noexcept
{
  ...
  std::vector<FloatPoint2D> vertices(n_contours * CONTOUR_VERTICES);
  ...
  indices.reserve((CONTOUR_VERTICES - 2) * 3 +
                  n_intervals * CONTOUR_VERTICES * 6);
  ...
  for (unsigned i = 0; i < n_intervals; ++i) {
    ...
    for (unsigned j = 0; j < CONTOUR_VERTICES; ++j) {
      ...
      indices.push_back(a + j2);

      indices.push_back(b + j);
      indices.push_back(b + j2);
      indices.push_back(a + j2);
    }
  }
