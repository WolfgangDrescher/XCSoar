...
/**
 * Draw a dashed outline along a round rectangle.  The dashes follow
 * the rounded corners: the path is walked by arc length, which the
 * canvas has no primitive for.
 */
static void
                                ...
                                ButtonState state) const noexcept
{
  ...
  /* No drop shadow: definition comes from a border on the face
     outline, like a Tailwind `ring ring-inset`.  It is drawn as a
     filled round rectangle with the face on top of it, not as a
     stroked outline: a stroke runs through the same integer vertices
     as the fill, so its width visibly wobbles around the corners
     where every vertex snaps to a whole pixel.  Two filled fans give
     a border of constant width instead. */
