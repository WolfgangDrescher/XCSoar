...
\item[Notifications]  (iOS devices only) If enabled, status messages are also
  shown as system notifications while XCSoar is not in the foreground, so that
  they appear on the lock screen.  The first start asks the operating system
  for permission; if you decline it there, this setting has no effect until you
  allow notifications for XCSoar in the iOS settings.
\end{description}
