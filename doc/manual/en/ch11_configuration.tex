...
This page provides options for how waypoints are displayed on the map. The
preview at the bottom of the page shows two airfields with the current
settings, one of them reachable and one of them not.
...
\item[Label visibility*]  \label{conf:labelvisibility} Controls which waypoints
  are displayed with a label on the map: \\
  {\bf All}: All waypoint labels will be displayed. \\
  {\bf Task waypoints \& airfields}: All waypoints part of a task and all
  airfields will be displayed. \\
  {\bf Task waypoints \& landables}: All waypoints part of a task and all
...
\item[Label style*] \label{conf:labelstyle} How waypoint labels are drawn. This
  applies to every waypoint label on the map. \\
  {\bf Text}: Plain text without an outline. \\
  {\bf Outlined text}: Black text with a white outline, readable on any
  background. (default option) \\
  {\bf Inverted text}: White text with a black outline. \\
  {\bf Badge}: Text on a rounded translucent background.

\item[Reachable label style*] \label{conf:labelhighlight} How the labels of the
  reachable landables are drawn; the task waypoints and the watched waypoints
  get the same treatment. Every
  option except the first one also draws them in bold. \\
  {\bf Same as others}: Nothing is highlighted. \\
  {\bf Bold}: The label style in bold. \\
  {\bf Outlined text}, {\bf Inverted text}, {\bf Badge}: As above, in bold.
  (default option: Badge)

\item[Arrival info*] \label{conf:arrivalheight} Which value is displayed with
  the waypoint label. See Section~\ref{sec:waypoint-schemes} for more
  information. \\
  {\bf None}: No value is displayed. \\
  {\bf Arrival height}: The height above the safety 'Arrival height'
  (\ref{sec:safety-factors}) at the waypoint. (default option) \\
  {\bf Arrival altitude}: The altitude at which the waypoint is reached, the
  safety 'Arrival height' included. \\
  {\bf Required glide ratio}: The glide ratio over the ground required to get
  there, as a whole number. It is shown whenever the aircraft is higher than
  that height, regardless of how shallow the glide is. \\
  {\bf Height \& glide ratio}, {\bf Altitude \& glide ratio},
  {\bf Height \& altitude}, {\bf Height, altitude \& glide ratio}: The
  combinations of the three values above.

  Several values are separated by a thin rule inside the label.

\item[Calculation*] How the arrival height is calculated. The required glide
  ratio is always calculated straight over the ground and ignores this
  setting. \\
  {\bf Straight glide}: Straight glide arrival height (no terrain is
  considered). (default option) \\
  {\bf Terrain avoidance glide}: Arrival height considering any detour needed
  to avoid terrain. \\
  {\bf Straight \& terrain glide}: Both arrival heights, but only where the
  detour costs at least 10 m and 5 \%; otherwise the straight glide arrival
  height alone.

  For the last two options, the 'Reach mode' setting (on the 'Glide Computer /
  Route' configuration page, ~\ref{sec:glide-comp-route}) must be set to
  'Turning'.

  \warning
  If 'Straight \& terrain glide' is selected and only one arrival height (a
  positive one) is shown for a waypoint (e.g., 'Moontown:100m'), either no
  detour around terrain is needed or there's interfering terrain and no
  possible detour will get you to the waypoint at an acceptable height. These
  are two \emph{very different situations} safety-wise!
...
\item[Arrival info position*] Where the value is drawn. \\
  {\bf After the name}: Inside the waypoint label, separated from the name by
  a thin rule. (default option) \\
  {\bf Badge below}: On a rounded label of its own below the waypoint label.
  It is the first label to be dropped when the map gets crowded.

\item[Show arrival info*] Which waypoints get the value. \\
  {\bf Reachable landables}: Only landables which can be reached, plus the
  watched waypoints. Negative arrival heights are shown for the watched
  waypoints only. (default option) \\
  {\bf All landables}: All landables, whether they can be reached or not,
  plus the watched waypoints. \\
  {\bf All labelled waypoints}: Every waypoint which has a label, including
  the ones which are not landable. This calculates a glide solution for every
  waypoint on the screen, which costs noticeably more on slow devices.
