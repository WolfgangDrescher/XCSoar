...
\item[Highlight*] \label{conf:labelhighlight} How the labels of the reachable
  landables, the task waypoints and the watched waypoints are drawn. Every
...
\item[Arrival info*] \label{conf:arrivalheight} Which value is displayed with
  ...
  {\bf Both}: The arrival height followed by the required glide ratio.
...
\item[Arrival info position*] Where the value is drawn. \\
  {\bf After the name}: Appended to the waypoint label, separated by a colon,
  as in 'Moontown:100m'. (default option) \\
...
\item[Show arrival info*] Which waypoints get the value. \\
  {\bf Reachable only}: Only waypoints which can be reached, plus the watched
  ones. Negative arrival heights are shown for the watched waypoints only.
  (default option) \\
  {\bf All labelled waypoints}: Every waypoint which has a label, including
  the unreachable ones and the ones which are not landable. Negative arrival
  heights are shown as well. This calculates a glide solution for every
