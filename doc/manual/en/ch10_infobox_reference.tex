...
\ibi{Spacer}{Spacer}{Not displayed and claims no space of its own: the other
InfoBoxes of the same row (or, in landscape, of the same column) grow into the
gap.  Use it for a row with fewer but wider InfoBoxes than the selected
geometry provides, for example four InfoBoxes in a row of five, without
switching to a different InfoBox geometry.}

\ibi{Merge with previous}{Merge}{Not displayed: the preceding InfoBox of the
same row (or, in landscape, of the same column) grows over it and gets twice
the space, which helps with long values such as waypoint names.  Several of
them in a row widen that InfoBox further.  In the first slot of a row it
behaves like a Spacer.}

\ibi{Invisible}{Invisible}{Draws nothing at all: the InfoBox slot stays
completely transparent and the map is extended below it, so the map remains
visible in its place.  Use this to leave a hole in the InfoBox grid without
switching to a different InfoBox geometry.  A short tap on an invisible
InfoBox acts on the map underneath; a long press opens the InfoBox selection
list, so a different InfoBox can be assigned to that slot again.}
